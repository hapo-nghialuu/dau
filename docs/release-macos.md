...
- **So sánh semver** giữa `tag_name` và `CFBundleShortVersionString` (bundle version, ví dụ `0.1.1`). Chỉ hiện update khi `tag_name > version` hiện tại.
...
1. Bump `MARKETING_VERSION` trong `platforms/macos/Dau.xcodeproj/project.pbxproj` đúng semver (hiện `0.1.1`).
